...
void IRCClient::SendMessage(const std::string &message, const std::string &global) const {
    if (!m_connected) return;

    std::string target = (global == "true") ? m_channel : global;
    std::string msg = "PRIVMSG " + target + " :" + message + "\r\n";
    send(m_socket, msg.c_str(), msg.size(), 0);
}

void IRCClient::Disconnect() {
    if (m_connected) {
        std::string quitCmd = "QUIT " + m_channel + "\r\n";
        send(m_socket, quitCmd.c_str(), quitCmd.size(), 0);
        closesocket(m_socket);
